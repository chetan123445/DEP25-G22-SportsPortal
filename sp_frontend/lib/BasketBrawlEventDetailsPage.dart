...
class BasketBrawlEventDetailsPage extends StatefulWidget {
  final dynamic event;
  final bool isReadOnly; // Add this parameter

  BasketBrawlEventDetailsPage({
    required this.event,
    this.isReadOnly = false, // Default to false for backward compatibility
  });

  @override
  _BasketBrawlEventDetailsPageState createState() =>
      _BasketBrawlEventDetailsPageState();
}

class _BasketBrawlEventDetailsPageState
    extends State<BasketBrawlEventDetailsPage>
  ...
  Future<void> deleteCommentary(String commentaryId) async {
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/basketbrawl/delete-commentary'), // Updated URL
  ...
  Future<void> fetchStandings([int? year]) async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          '$baseUrl/basketbrawl/standings${yearParam.isNotEmpty ? "?year=$yearParam" : ""}',
        ),
      );
  ...
  Future<void> addCommentary(String text) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/basketbrawl/add-commentary'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'eventId': widget.event['_id'],
          'text': text,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('BasketBrawl Event Details'),
